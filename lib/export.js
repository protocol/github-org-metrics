...
const getRepos = async function * (org, token) {
  const get = bent({
  ...
  const print = () => repos.forEach(n => console.log(n))
  print()
  ...
  while (links && links.next) {
    ...
    repos = (await getJSON(resp)).map(r => r.full_name)
    print()
...
const exportData = async argv => {
  ...
  let repos = await Promise.all(getRepos(org, token))
  // remove any possible duplicates and sort
  repos = Array.from(new Set(repos)).sort()
